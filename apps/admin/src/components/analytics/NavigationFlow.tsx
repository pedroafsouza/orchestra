...
export function NavigationFlow({ paths }: NavigationFlowProps) {
            ...
            {sorted.map((p) => (
              ...
              >
                <div className="flex items-center gap-1.5 flex-1 min-w-0">
                  <span className="px-2 py-1 rounded bg-primary/10 text-primary font-medium truncate">
                    {p.source}
                  </span>
                  <ArrowRight className="h-3 w-3 text-muted-foreground shrink-0" />
                  <span className="px-2 py-1 rounded bg-violet-500/10 text-violet-600 dark:text-violet-400 font-medium truncate">
                    {p.target}
                  </span>
                </div>
